...
void TestTemporaryObjConstructor() {
    ...
    std::cout << "Test with temporary object, copy elision"
              << std::endl;
...
void TestTemporaryObjOperator() {
    const size_t size = 1000000;
    std::cout << "Test with temporary object, operator="
              << std::endl;
...
void TestNamedMoveConstructor() {
    const size_t size = 1000000;
    std::cout << "Test with named object, move constructor"
              << std::endl;
...
void TestNamedMoveOperator() {
    const size_t size = 1000000;
    std::cout << "Test with named object, operator="
              << std::endl;
    SimpleVector<int> vector_to_move(GenerateVector(size));
    assert(vector_to_move.GetSize() == size);
...
void TestNoncopiableMoveConstructor() {
    const size_t size = 5;
    std::cout << "Test noncopiable object, move constructor"
              << std::endl;
...
void TestNoncopiablePushBack() {
    const size_t size = 5;
    std::cout << "Test noncopiable push back"
              << std::endl;
    SimpleVector<X> v;
    for (size_t i = 0; i < size; ++i) {
        v.PushBack(X(i));
...
void TestNoncopiableErase() {
    const size_t size = 3;
    std::cout << "Test noncopiable erase"
              << std::endl;
    SimpleVector<X> v;
    for (size_t i = 0; i < size; ++i) {
        v.PushBack(X(i));
